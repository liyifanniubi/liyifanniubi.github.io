...
<!-- ##{"style":"<style>p span {position: relative;background-color: rgba(200, 200, 200, 0.3);color: rgba(0, 0, 0, 0.5);user-select: none;overflow: hidden;cursor: pointer;filter: blur(3px);padding: 5px;transition: filter 0.3s ease;}p span:hover {background-color: inherit;color: inherit;filter: blur(0);
}</style>"}## -->